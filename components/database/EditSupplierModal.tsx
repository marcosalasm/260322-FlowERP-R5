...
export const EditSupplierModal: React.FC<EditSupplierModalProps> = ({
    ...
    useEffect(() => {
        if (supplier && isOpen) {
            ...
            setServiceType(supplier.serviceType || '');
            setLocation(supplier.location || '');
            setPhone(supplier.phone || '');
            setEmail(supplier.email || '');
            setSinpePhone(supplier.sinpePhone || '');
            setBankAccount(supplier.bankAccount || '');
            setBankAccountDetails(supplier.bankAccountDetails || '');
            setBankAccount2(supplier.bankAccount2 || '');
            setBankAccount2Details(supplier.bankAccount2Details || '');
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" role="dialog" aria-modal="true">
            <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-2xl transform transition-all max-h-[95vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-dark-gray">Editar Proveedor</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                     <div>
                        <label htmlFor="edit-name" className="block text-sm font-bold text-slate-700 mb-1">Nombre del Proveedor</label>
                    ...
                    <div>
                        <label htmlFor="edit-serviceType" className="block text-sm font-bold text-slate-700 mb-1">Bien o Servicio Principal</label>
                        <select 
                            id="edit-serviceType" 
                            value={serviceType} 
                    ...
                    <div>
                        <label htmlFor="edit-location" className="block text-sm font-bold text-slate-700 mb-1">Ubicación</label>
                        <input type="text" id="edit-location" value={location} onChange={e => setLocation(e.target.value)} required className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                           <label htmlFor="edit-phone" className="block text-sm font-bold text-slate-700 mb-1">Teléfono</label>
                        ...
                        <div>
                            <label htmlFor="edit-email" className="block text-sm font-bold text-slate-400 mb-1">Correo Electrónico <span className="font-normal">(Opcional)</span></label>
                    ...
                    <div className="pt-4 mt-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <div>
                                <label htmlFor="edit-sinpePhone" className="block text-sm font-bold text-slate-700 mb-1">Teléfono (SINPE)</label>
                                <input type="tel" id="edit-sinpePhone" value={sinpePhone} onChange={e => setSinpePhone(e.target.value)} className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Número para SINPE móvil" />
                            </div>
                            <div>
                                <label htmlFor="edit-bankAccountPrimary" className="block text-sm font-bold text-slate-700 mb-1">Cuenta Bancaria Principal (IBAN)</label>
                                <input type="text" id="edit-bankAccountPrimary" value={bankAccount} onChange={e => setBankAccount(e.target.value)} className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary" placeholder="CR..." />
                            </div>
                        </div>
                    ...
                    <div className="pt-6 border-t border-slate-200">
                        <label htmlFor="edit-comments" className="block text-sm font-bold text-slate-700 mb-1">Comentarios / Retroalimentación</label>
                        <textarea 
                            id="edit-comments" 
                            value={comments} 
                    ...
                    <div className="flex justify-end gap-4 pt-6 border-t border-light-gray">
                        ...
                        <button type="submit" disabled={isSubmitting} className="bg-primary text-white font-bold py-2 px-8 rounded-lg hover:bg-primary-dark transition-colors disabled:bg-slate-400 shadow-lg">
