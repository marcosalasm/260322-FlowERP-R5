...
import { Material, ServiceItem, RecurringOrderTemplate, ServiceRequestItem } from '../../types';
...
interface EditRecurringOrderModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (updatedTemplate: RecurringOrderTemplate) => void;
    template: RecurringOrderTemplate | null;
    materials: Material[];
...
export const EditRecurringOrderModal: React.FC<EditRecurringOrderModalProps> = ({
    isOpen,
    onClose,
    onSubmit,
    template,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!name || finalItems.length === 0) {
            alert('Por favor complete el nombre de la plantilla y agregue al menos un artículo válido.');
            setIsSubmitting(false);
            return;
        }

        const updatedTemplate: RecurringOrderTemplate = {
            ...template,
            name,
            description,
            items: finalItems,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" role="dialog" aria-modal="true">
            <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-4xl transform transition-all h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6 flex-shrink-0">
                    <h2 className="text-2xl font-bold text-dark-gray">Editar Pedido Recurrente</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 flex-grow flex flex-col overflow-hidden">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-shrink-0">
                        <div>
                            <label htmlFor="edit-template-name" className="block text-sm font-medium text-slate-700 mb-1">Nombre de la Plantilla</label>
                            <input type="text" id="edit-template-name" value={name} onChange={e => setName(e.target.value)} required className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary text-sm" />
                        </div>
                         <div>
                            <label htmlFor="edit-template-desc" className="block text-sm font-medium text-slate-700 mb-1">Descripción (Opcional)</label>
                            <input type="text" id="edit-template-desc" value={description} onChange={e => setDescription(e.target.value)} className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary text-sm" />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col flex-grow overflow-hidden">
                        ...
                        <div className="space-y-3 overflow-y-auto flex-grow pr-2 -mr-2 border rounded-md p-4 bg-slate-50 shadow-inner">
                            {items.map((item, index) => (
                                <div key={item.id} className="grid grid-cols-[1fr,100px,100px,auto] gap-2 items-center animate-fade-in">
                                    <input 
                                        type="text" 
                                        list="edit-item-catalog-template"
                        ...
                        <datalist id="edit-item-catalog-template">
