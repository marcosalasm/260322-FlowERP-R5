...
import { Project, ChangeOrderStatus, Budget, BudgetStatus, IndirectCosts } from '../../types';
...
interface NewChangeOrderModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (newChangeOrderData: any) => void;
    executableProjects: Project[];
    budgets: Budget[];
}

export const NewChangeOrderModal: React.FC<NewChangeOrderModalProps> = ({
    isOpen,
    onClose,
    onSubmit,
    executableProjects,
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setSelectedProjectId(executableProjects.length > 0 ? String(executableProjects[0].id) : '');
            setSelectedBudgetId(availableBudgets.length > 0 ? String(availableBudgets[0].id) : '');
            setChangeType('Adicional');
            setCreationDate(format(new Date(), 'dd/MM/yyyy HH:mm'));
        }
    }, [isOpen, executableProjects, availableBudgets]);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        setIsSubmitting(true);

        const selectedProject = executableProjects.find(p => p.id === parseInt(selectedProjectId, 10));
        if (!selectedProject || !selectedProject.offerId) {
            alert('Por favor seleccione un proyecto válido.');
        ...
        const newChangeOrderData = {
            ...impactData,
            offerId: selectedProject.offerId,
            projectName: selectedProject.name,
            description,
            creationDate: creationDate,
            status: ChangeOrderStatus.PendingApproval,
        };

        onSubmit(newChangeOrderData);
        onClose();
        setIsSubmitting(false);
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden transform transition-all" onClick={e => e.stopPropagation()}>
                {/* Header */}
                <div className="flex justify-between items-center p-6 sm:p-8 border-b flex-shrink-0">
                    <h2 className="text-2xl font-bold text-dark-gray">Nueva Orden de Cambio</h2>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-grow overflow-hidden">
                    <div className="flex-grow overflow-y-auto p-6 sm:p-8 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label htmlFor="co-project" className="block text-sm font-medium text-slate-700 mb-1">Vincular a Proyecto</label>
                                <select id="co-project" value={selectedProjectId} onChange={e => setSelectedProjectId(e.target.value)} required className="w-full p-2 border border-slate-300 rounded-md shadow-sm">
                                    {executableProjects.length > 0 ?
                                        executableProjects.map(p => <option key={p.id} value={p.id}>{`#${p.id} - ${p.name}`}</option>) :
                                        <option disabled>No hay proyectos "En Ejecución"</option>
                                    }
                                </select>
                            </div>
                            <div>
                                <label htmlFor="co-creation-date" className="block text-sm font-medium text-slate-700 mb-1">Fecha de Creación</label>
                                <input id="co-creation-date" type="text" value={creationDate} readOnly className="w-full p-2 border border-slate-200 bg-slate-100 rounded-md shadow-sm" />
                            </div>
                        </div>
                        <div>
                            <label htmlFor="co-description" className="block text-sm font-medium text-slate-700 mb-1">Descripción del Cambio</label>
                            <textarea id="co-description" rows={3} value={description} onChange={e => setDescription(e.target.value)} required className="w-full p-2 border border-slate-300 rounded-md shadow-sm" />
                        </div>
                        <div>
                            ...
                            <div className="flex gap-4">
                                <label className="flex items-center p-2 border rounded-md cursor-pointer w-full">
                                    <input type="radio" name="changeType" value="Adicional" checked={changeType === 'Adicional'} onChange={() => setChangeType('Adicional')} className="h-4 w-4 text-primary focus:ring-primary" />
                                    <span className="ml-2 text-sm font-medium text-slate-700">Adicional (Aumenta el valor)</span>
                                </label>
                                <label className="flex items-center p-2 border rounded-md cursor-pointer w-full">
                                    <input type="radio" name="changeType" value="Crédito" checked={changeType === 'Crédito'} onChange={() => setChangeType('Crédito')} className="h-4 w-4 text-primary focus:ring-primary" />
                        ...
                        {entryMode === 'link' ? (
                            <div className="space-y-4 animate-fade-in">
                                <select value={selectedBudgetId} onChange={e => setSelectedBudgetId(e.target.value)} className="w-full p-2 border border-slate-300 rounded-md" disabled={availableBudgets.length === 0}>
                                    {availableBudgets.length > 0 ? availableBudgets.map(b => <option key={b.id} value={b.id}>{b.consecutiveNumber} - {b.description || 'Sin descripción'} - {b.prospectName}</option>) : <option disabled>No hay presupuestos disponibles</option>}
                                </select>
                                {selectedBudget && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
                                <div>
                                    <label htmlFor="manual-amount-impact" className="block text-sm font-medium text-slate-700 mb-1">Impacto en Monto (¢)</label>
                                    <input type="number" id="manual-amount-impact" value={manualAmountImpact} onChange={e => setManualAmountImpact(e.target.value)} required placeholder="Ej: 5000" min="0" className="w-full p-2 border border-slate-300 rounded-md" />
                                </div>
                                <div>
                                    <label htmlFor="manual-budget-impact" className="block text-sm font-medium text-slate-700 mb-1">Impacto en Presupuesto (¢)</label>
                                    <input type="number" id="manual-budget-impact" value={manualBudgetImpact} onChange={e => setManualBudgetImpact(e.target.value)} required placeholder="Ej: 3000" min="0" className="w-full p-2 border border-slate-300 rounded-md" />
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="p-6 border-t flex justify-end gap-4 bg-slate-50 flex-shrink-0">
                        ...
                        <button type="submit" disabled={isSubmitting || executableProjects.length === 0} className="bg-primary text-white font-bold py-2 px-6 rounded-lg hover:bg-primary-dark disabled:bg-slate-400 shadow-lg">Crear Orden</button>
                    </div>
                </form>
            </div>
        </div>
