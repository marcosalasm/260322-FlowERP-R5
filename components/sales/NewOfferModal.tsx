...
import { Prospect, OfferStatus, ProjectType, Budget, BudgetStatus, IndirectCosts } from '../../types';
import { formatNumber } from '../../utils/format';

interface NewOfferModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (newOfferData: any) => void;
    prospects: Prospect[];
    budgets: Budget[];
}
...
export const NewOfferModal: React.FC<NewOfferModalProps> = ({
    ...
    const [manualProspectId, setManualProspectId] = useState<string>('');
    ...
    const availableBudgets = useMemo(() => {
        // A non-recurring budget is available if Finalized.
        // A recurring budget is available if Finalized.
        // The status change logic in SalesDashboard handles making non-recurring ones unavailable after use.
        return budgets.filter(b => b.status === BudgetStatus.Finalized);
    }, [budgets]);
    ...
    useEffect(() => {
        if (entryMode === 'link' && selectedBudget) {
            setDescription(selectedBudget.description || '');
        } else if (entryMode === 'manual') {
            setDescription(''); // Clear when switching to manual
        }
    }, [selectedBudget, entryMode]);
    ...
    const resetForm = () => {
        setEntryMode('link');
        setSelectedBudgetId(availableBudgets.length > 0 ? String(availableBudgets[0].id) : '');
        setManualProspectId(prospects.length > 0 ? String(prospects[0].id) : '');
        setManualAmount('');
        setManualBudget('');
        setDescription('');
        setProjectType(ProjectType.Construccion);
        setAttachment(null);
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (entryMode === 'link') {
            ...
            newOfferData = {
                prospectId: selectedBudget.prospectId,
                prospectName: selectedBudget.prospectName,
                amount: selectedBudget.finalTotal,
                budget: selectedBudget.finalTotal - calculatedCosts.totalDeduction,
                budgetId: selectedBudget.id,
            };
        } else { // Manual mode
            const selectedProspect = prospects.find(p => p.id === parseInt(manualProspectId, 10));
            if (!selectedProspect) {
                alert('Por favor, seleccione un prospecto válido.');
                setIsSubmitting(false);
                return;
            }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" role="dialog" aria-modal="true">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] transform transition-all flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center p-6 sm:p-8 flex-shrink-0 border-b">
                    <h2 className="text-2xl font-bold text-dark-gray">Nueva Oferta Comercial</h2>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-grow overflow-hidden">
                    <div className="flex-grow overflow-y-auto p-6 sm:p-8 space-y-6 custom-scrollbar">
                        ...
                        {entryMode === 'link' ? (
                            <div className="space-y-4 animate-fade-in">
                                <div>
                                    <label htmlFor="budget-selection" className="block text-sm font-medium text-slate-700 mb-1">Seleccionar Presupuesto</label>
                                    <select
                                        id="budget-selection"
                                    ...
                                    >
                                        {availableBudgets.length > 0 ? (
                                            ...
                                            <option value="" disabled>No hay presupuestos finalizados disponibles</option>
                                        )}
                                    </select>
                                </div>
                                {selectedBudget && (
                            ...
                            <div className="space-y-4 animate-fade-in">
                                <div>
                                    <label htmlFor="manual-prospect" className="block text-sm font-medium text-slate-700 mb-1">Prospecto</label>
                                    <select id="manual-prospect" value={manualProspectId} onChange={e => setManualProspectId(e.target.value)} required className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                        {prospects.map(p => <option key={p.id} value={p.id}>{p.company}</option>)}
                                    </select>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="manual-amount" className="block text-sm font-medium text-slate-700 mb-1">Monto de Oferta</label>
                                        <input type="number" id="manual-amount" value={manualAmount} min="0" onChange={e => setManualAmount(e.target.value)} required className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary" />
                                    </div>
                                    <div>
                                        <label htmlFor="manual-budget" className="block text-sm font-medium text-slate-700 mb-1">Costo de Presupuesto</label>
                                        <input type="number" id="manual-budget" value={manualBudget} min="0" onChange={e => setManualBudget(e.target.value)} required className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary" />
                                    </div>
                                </div>
                            </div>
                        )}
                        <div>
                            <label htmlFor="offer-description" className="block text-sm font-medium text-slate-700 mb-1">Descripción de la Oferta</label>
                            <textarea id="offer-description" value={description} onChange={e => setDescription(e.target.value)} rows={3} className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Resumen del alcance, notas importantes, etc." />
                        </div>
                        <div>
                            <label htmlFor="projectType" className="block text-sm font-medium text-slate-700 mb-1">Tipo de Proyecto</label>
                            <select id="projectType" value={projectType} onChange={e => setProjectType(e.target.value as ProjectType)} className="w-full p-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary">
                            ...
                            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-md">
                                <div className="space-y-1 text-center">
                                    ...
                                    <p className="text-xs text-slate-500">
                                        {attachment ? `Archivo seleccionado: ${attachment.name}` : 'PDF hasta 10MB'}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex justify-end gap-4 p-6 sm:p-8 flex-shrink-0 border-t bg-slate-50">
