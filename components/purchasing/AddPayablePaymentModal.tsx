...
import { AccountPayable, Payment } from '../../types';
...
interface AddPayablePaymentModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export const AddPayablePaymentModal: React.FC<AddPayablePaymentModalProps> = ({ isOpen, onClose, account, onSubmit }) => {
    ...
    const linkedSubcontract = useMemo(() => {
        if (!account || !account.subcontractId || !appContext) return null;
        return appContext.subcontracts.find(s => s.id === account.subcontractId);
    }, [account, appContext]);

    // Calcular el saldo real pendiente considerando abonos por notas de crédito
    const receivableAmount = useMemo(() => {
        if (!account) return 0;
        return account.totalAmount - account.paidAmount - (account.creditedAmount || 0);
    }, [account]);
    ...
    const handleInstallmentSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (instId && linkedSubcontract?.installments) {
            const inst = linkedSubcontract.installments.find(i => i.id === Number(instId));
            if (inst) {
                setAmount(String(inst.amount));
                setDetails(`Pago de ${inst.description}`);
    ...
    const handleFillTotalAmount = () => {
        setAmount(String(receivableAmount));
        setError('');
    };
    ...
    const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (numericValue > receivableAmount) {
            setError(`El monto no puede exceder el saldo de ¢${formatCurrency(receivableAmount)}`);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose} role="dialog" aria-modal="true">
            <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-lg transform transition-all" onClick={e => e.stopPropagation()}>
                ...
                <p className="text-sm text-slate-600 mb-1">Proveedor: <span className="font-medium text-slate-800">{account.supplierName}</span></p>
                <p className="text-sm text-slate-600 mb-4">Saldo Pendiente: <span className="font-semibold text-red-600">¢{formatCurrency(receivableAmount)}</span></p>

                <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Installment Selector for Subcontracts */}
                    {linkedSubcontract && linkedSubcontract.installments && linkedSubcontract.installments.length > 0 && (
                        <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 animate-fade-in">
                            <label htmlFor="installment-select" className="block text-xs font-black text-blue-800 uppercase tracking-widest mb-1.5">Pago de Tracto Programado</label>
                            <select
                                id="installment-select"
                                value={selectedInstallmentId}
                                onChange={handleInstallmentSelect}
                                className="w-full p-2 border border-blue-200 rounded-md text-sm font-semibold text-blue-900 bg-white"
                            >
                                <option value="">- Seleccione un tracto del contrato -</option>
                                {linkedSubcontract.installments.map(inst => (
                                    <option key={inst.id} value={inst.id}>
                                        {inst.description} - ¢{formatCurrency(inst.amount)}
                                    </option>
                                ))}
                            </select>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1">
                                <label htmlFor="payment-amount" className="block text-sm font-medium text-slate-700">Monto a Pagar (¢)</label>
                                <button
                                    type="button"
                                    onClick={handleFillTotalAmount}
                                    className="text-[10px] font-bold text-primary hover:text-primary-dark uppercase tracking-tighter"
                                >
                                    Pagar Saldo
                                </button>
                            </div>
